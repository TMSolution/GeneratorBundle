...
use TMSolution\GeneratorBundle\Generator\DoctrineCrudGenerator;
    ...
    protected function configure()
    {
        $this
            ->setDefinition(array(
                ...
                new InputOption('formstyle','', InputOption::VALUE_REQUIRED, 'Use form format(horizontal, inline or empty for base form )', '')
            ))
...
EOT
            ...
            ->setName('generate:tmsolution:crud');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $formstyle   = $input->getOption('formstyle');
        $input->setOption('formstyle', $formstyle);
        $dialog->writeSection($output, 'CRUD generation');
        $entityClass = $this->getContainer()->get('doctrine')->getAliasNamespace($bundle).'\\'.$entity;
        $metadata    = $this->getEntityMetadata($entityClass);
        $bundle      = $this->getContainer()->get('kernel')->getBundle($bundle);
        $generator = $this->getGenerator($bundle);
        $generator->generate($bundle, $entity, $metadata[0], $format, $prefix, $withWrite, $forceOverwrite, $formstyle);
        ...
        // form
        if ($withWrite) {
            $this->generateForm($bundle, $entity, $metadata, $formstyle);
     ...
     */
    protected function generateForm($bundle, $entity, $metadata, $formstyle)
    {
        //var_dump($formstyle);
        try {
            $this->getFormGenerator($bundle)->generate($bundle, $entity, $metadata[0], $formstyle);
